...
def test_indent():
    ...
    assert ds.indent(''.join(ds.getsourcelines(tm.quadratic, lstrip=True)[
                     0]), 2) == ''.join(ds.getsourcelines(tm.quadratic)[0])
...
def test_name():
    ...
    assert ds._namespace(ts.squared) == ds.getname(
        ts.squared, fqn=True).split('.')
