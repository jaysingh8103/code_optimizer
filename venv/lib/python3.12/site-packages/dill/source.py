...
def _matchlambda(func, line):
    ...
    def dummy(): return '__this_is_a_big_dummy_function__'
    ...
    _ = dict(re.match(r'([\W\D\S])(.*)', _[i]).groups()
             for i in range(1, len(_)))
    _f = dict(re.match(r'([\W\D\S])(.*)', _f[i]).groups()
...
def getsourcelines(object, lstrip=False, enclosing=False):
    ...
    If lstrip=True, ensure there is no indentation in the first line of code.
    If enclosing=True, then also return any enclosing code."""
    code, n = getblocks(object, lstrip=lstrip,
                        enclosing=enclosing, locate=True)
    return code[-1], n[-1]
...
def _namespace(obj):
    ...
    try:  # XXX: needs some work and testing on different types
        module = qual = str(getmodule(obj)).split()[
            1].strip('>').strip('"').strip("'")
        qual = qual.split('.')
        if ismodule(obj):
            return qual
...
def importable(obj, alias='', source=None, builtin=True):
    ...
    while True:
        ...
        try:
            ...
            obj = globalvars(obj)
            obj = list(getsource(_obj, name, force=True)
