        ...
        if NumpyArrayType and not (obj_type is type or obj_type in Pickler.dispatch):
            ...
            if numpydtype(obj_type):
                ...
                def save_numpy_dtype(pickler, obj):
                    ...
                    pickler.save_reduce(_create_dtypemeta,
                                        (obj.type,), obj=obj)
    ...
    with match(args) as m:
        ...
        if m.case((
            ...
            if CODE_VERSION == (3, 11):
                return CodeType(
                    *args[:6],
                    args[6].encode() if hasattr(
                        args[6], 'encode') else args[6],  # code
                    *args[7:14],
                    args[14].encode() if hasattr(
                        args[14], 'encode') else args[14],  # linetable
                    args[15].encode() if hasattr(
                        args[15], 'encode') else args[15],  # exceptiontable
                    args[16],
                    args[17],
                )
            fields = m.fields
        # Python 3.10 or 3.8/3.9 (16 members)
            ...
            # args[13:]
            'LNOTAB_OR_LINETABLE', 'freevars', 'cellvars'
        )):
                ...
                return CodeType(
                    *args[:6],
                    args[6].encode() if hasattr(
                        args[6], 'encode') else args[6],  # code
                    *args[7:13],
                    args[13].encode() if hasattr(
                        args[13], 'encode') else args[13],  # lnotab/linetable
                    args[14],
                    args[15],
                )
            fields = m.fields
        ...
        # Python 3.7 (15 args)
            ...
            if CODE_VERSION == (3, 7):
                return CodeType(
                    *args[:5],
                    args[5].encode() if hasattr(
                        args[5], 'encode') else args[5],  # code
                    *args[6:12],
                    args[12].encode() if hasattr(
                        args[12], 'encode') else args[12],  # lnotab
                    args[13],
                    args[14],
                )
            fields = m.fields
        # Python 3.11a (20 members)
            ...
            'code', 'consts', 'names', 'varnames', 'filename', 'name', 'qualname', 'firstlineno',
            # args[14:]
            'linetable', 'endlinetable', 'columntable', 'exceptiontable', 'freevars', 'cellvars'
        )):
            if CODE_VERSION == (3, 11, 'a'):
                return CodeType(
                    ...
                    *(a.encode() if hasattr(a, 'encode')
                      else a for a in args[14:18]),
                    args[18],
                    args[19],
                )
            fields = m.fields
...
try:
    ...
    MAPPING_PROXY_TRICK = __d is (
        DictProxyType(__d) | _dictproxy_helper_instance)
except Exception:
...
def _create_namedtuple(name, fieldnames, modulename, defaults=None):
    ...
    t = collections.namedtuple(
        name, fieldnames, defaults=defaults, module=modulename)
    return t
...
def _create_capsule(pointer, name, context, destructor):
    ...
    if attr_found:
        ...
        raise UnpicklingError(
            "%s object exists at %s but a PyCapsule object was expected." % (type(capsule), name))
    else:
...
def _save_with_postproc(pickler, reduction, is_pickler_dill=None, obj=Getattr.NO_DEFAULT, postproc_list=None):
    ...
    if is_pickler_dill:
        ...
        if id(obj) in pickler._postproc:
            name = '%s.%s ' % (obj.__module__, getattr(
                obj, '__qualname__', obj.__name__)) if hasattr(obj, '__module__') else ''
            warnings.warn('Cannot pickle %r: %shas recursive self-references that trigger a RecursionError.' %
...
if ThreadHandleType:
    ...
    def save_thread_handle(pickler, obj):
        ...
        pickler.save_reduce(_create_thread_handle,
                            (obj.ident, obj.is_done()), obj=obj)
...
def _is_builtin_module(module):
    ...
    names = ["base_prefix", "base_exec_prefix",
             "exec_prefix", "prefix", "real_prefix"]
    rp = os.path.realpath
...
def save_type(pickler, obj, postproc_list=None):
    if obj in _typemap:
        ...
        if not obj._field_defaults:
            _save_with_postproc(pickler, (_create_namedtuple, (obj.__name__,
                                obj._fields, obj.__module__)), obj=obj, postproc_list=postproc_list)
        else:
            defaults = [obj._field_defaults[field]
                        for field in obj._fields if field in obj._field_defaults]
            _save_with_postproc(pickler, (_create_namedtuple, (obj.__name__, obj._fields,
                                obj.__module__, defaults)), obj=obj, postproc_list=postproc_list)
    ...
    # special caes: NoneType, NotImplementedType, EllipsisType, EnumMeta, etc
        ...
        if not _byref and not obj_recursive and incorrectly_named:  # not a function, but the name was held over
            ...
            _dict, attrs = _get_typedict_type(
                obj, obj.__dict__.copy(), None, postproc_list)  # copy dict proxy to a dict
           ...
           # print ("%s\n%s" % (obj.__bases__, obj.__dict__))
            ...
            if isinstance(obj, abc.ABCMeta):
                ...
                _dict, attrs = _get_typedict_abc(
                    obj, _dict, attrs, postproc_list)
...
def save_function(pickler, obj):
    if not _locate_function(obj, pickler):
        ...
        _original_main = getattr(
            pickler, '_original_main', __builtin__)  # 'None'
        postproc_list = []
        if _recurse:
...
if HAS_CTYPES and hasattr(ctypes, 'pythonapi'):
    ...
    def save_capsule(pickler, obj):
        ...
        pickler.save_reduce(_create_capsule, (pointer, name,
                            context, destructor), obj=obj)
...
def pickles(obj, exact=False, safe=False, **kwds):
    ...
    if safe:
        ...
        exceptions = (TypeError, AssertionError,
                      NotImplementedError, PicklingError, UnpicklingError)
    try:
        pik = copy(obj, **kwds)
...
def check(obj, *args, **kwds):
    ...
    *python* is the path to the python interpreter (defaults to sys.executable)
    ...
    msg = "SUCCESS" if not subprocess.call(
        msg.split(None, 2)) else "LOAD FAILED"
    if verbose:
        print(msg)
    return
