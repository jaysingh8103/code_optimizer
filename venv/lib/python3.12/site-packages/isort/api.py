    ...
    **config_kwargs: Any,
    ...
    if config.atomic:
        try:
            ...
            compile(file_content, content_source,
                    "exec", flags=0, dont_inherit=True)
        except SyntaxError:
    ...
    if config.atomic:
        ...
        try:
            compile(_internal_output.read(), content_source,
                    "exec", flags=0, dont_inherit=True)
            _internal_output.seek(0)
    ...
    **config_kwargs: Any,
    ...
    with io.File.read(filename) as source_file:
        ...
        config = _config(path=actual_file_path,
                         config=file_config, **config_kwargs)
        changed: bool = False
        try:
            if write_to_stdout:
                changed = sort_stream(
                            ...
                            if changed:
                                if show_diff or ask_to_apply:
                                    ...
                                    show_unified_diff(
                                        ...
                                        file_path=actual_file_path,
                                        output=(
                                            None if show_diff is True else cast(
                                                TextIO, show_diff)
                                        ),
                                        color_output=config.color_output,
                                    )
