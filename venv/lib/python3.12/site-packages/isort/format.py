...
def remove_whitespace(content: str, line_separator: str = "\n") -> str:
    content = content.replace(line_separator, "").replace(
        " ", "").replace("\x0c", "")
    return content
...
class BasicPrinter:
    ...
    def success(self, message: str) -> None:
        print(self.success_message.format(
            success=self.SUCCESS, message=message), file=self.output)

    def error(self, message: str) -> None:
        print(self.error_message.format(
            error=self.ERROR, message=message), file=sys.stderr)
...
def create_terminal_printer(
    ...
    return (
        ColoramaPrinter(error, success, output) if color else BasicPrinter(
